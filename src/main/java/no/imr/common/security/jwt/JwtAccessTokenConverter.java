...
public class JwtAccessTokenConverter implements TokenEnhancer, AccessTokenConverter,
     ...
     * Class logger.
     */
    private static final Logger logger = LoggerFactory.getLogger(org.springframework.security.oauth2.provider.token.store.JwtAccessTokenConverter.class);
    ...
    /**
     * This is the key used for signature verification.
     */
     ...
     * Initalizes.
     *
     * @param cerfile This is the public certificate which contains the public key.
     * @throws IOException
     * @throws CertificateException
     * @throws InvalidKeyException
     */
    public JwtAccessTokenConverter(String cerfile) throws IOException, CertificateException, InvalidKeyException {
        String x509Data = IOUtils.toString(Thread.currentThread().getContextClassLoader().getResource(cerfile));
        CertificateFactory cf = CertificateFactory.getInstance("X.509");
        Certificate x509 = cf.generateCertificate(IOUtils.toInputStream(x509Data));
        PublicKey pk = x509.getPublicKey();
        RSAPublicKey rsaPK = new RSAPublicKeyImpl(pk.getEncoded());
        signatureVerifier = new RsaVerifier(rsaPK);
    }

    /**
     * @param tokenConverter the tokenConverter to set
     */
    public void setAccessTokenConverter(DefaultAccessTokenConverter tokenConverter) {
     ...
     */
    public DefaultAccessTokenConverter getAccessTokenConverter() {
